...
int	ft_handle(char c, va_list arg)
{
	...
	res = 0;
	if (c == '%')
		res = p_putchar_fd('%', 1, 0);
	if (c == 'c')
		res = p_putchar_fd((char)va_arg(arg, int), 1, 0);
	if (c == 's')
		res = p_putstr_fd((char *)va_arg(arg, char *), 1);
	if (c == 'p')
		res = ft_print_ptr((void *)va_arg(arg, void *));
	if (c == 'd' || c == 'i' || c == 'u')
		p_putnbr_fd((long)va_arg(arg, int), 1, c == 'u', &res);
	if (c == 'x')
